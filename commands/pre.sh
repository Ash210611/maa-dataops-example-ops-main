...
echo "UNSETTING TERRAGRUNT_IAM_ROLE"
...
function set_and_fed() {
    ...
    # Localhost federation process specified separately for Health Services and Cigna users
...
if [ $SOLUTION_BRANCH == "main" ] || [ $SOLUTION_BRANCH == "test" ] || [ $SOLUTION_BRANCH == "dev" ]
  then
    export BRANCH_WORKSPACE="default"
  else
    export BRANCH_WORKSPACE=$(echo ${SOLUTION_BRANCH#*/} | cut -d "_" -f1 --complement)
fi
