...
run_poetry_command() {
  ...
  poetry -C scripts/airflow_dag_runner run trigger_dag_and_monitor --mwaa_env=${MWAA_ENV} --region=${REGION} --dag_id=${dag_id} --s3_path=${s3_path} --secrets_manager_name=${SECRETS_MANAGER} --bucket_env=${ENV}
}
